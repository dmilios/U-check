...
public class GPEP_legacy extends AbstractGP<ClassificationPosterior> {

	private double eps_damp = 0.5;
	private int scale = 1;
	private double covarianceCorrection = 1e-4;

	public GPEP_legacy(KernelFunction kernel) {
		super(kernel);
	}
	...
	private double computeMarginalMoments(Gauss gauss, DoubleMatrix Term) {
		...
		DoubleMatrix A = gauss.LC_t.mmul(tmp).add(
				DoubleMatrix.eye(N).mul(CORRECTION_FACTOR));
		...
		gauss.W = Solve.solve(gauss.L, gauss.LC_t);

		// Gauss.diagV = sum(Gauss.W.*Gauss.W,1)';
		tmp = gauss.W.mul(gauss.W);
		gauss.diagV = new DoubleMatrix(N, 1);
		for (int i = 0; i < N; i++)
	...
	public static void main(String[] args) {
		...
		GPEP_legacy gp = new GPEP_legacy(new KernelRBF(1, 1));
